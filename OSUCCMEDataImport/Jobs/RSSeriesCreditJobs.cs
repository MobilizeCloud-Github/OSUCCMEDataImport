...
{
    class RSSeriesCreditJobs
    {
        public static void Process(string ImportUserID)
        {
            RSSeriesCredits(ImportUserID);
            //Console.WriteLine("");
            //Console.WriteLine("-----------------------------------");
            //Console.WriteLine("");
        ...
        private static void RSSeriesCredits(string ImportUserID)
        {
            ...
            try
            {
                ...
                Console.Write("Importing RSSeries User Credits - Starting ");
                Console.WriteLine(Total + " to Process");
                var Index = 1;

                foreach (var u in UserCreditsToImport)
                {
                    Console.Write("Processing RSSeries User Credits : (" + Index + "/" + Total + ") " + u.UserID);
                    ...
                    if (CommonFunctions.DoesUserExist(db, u.UserID))
                    {
                        ...
                        foreach (var uc in u.UserCredits)
                        {
                            ...
                            var Registration = (from c in db.RSSeriesRegistrations
                                                where c.RSSeriesID == uc.EventID && c.UserID == uc.UserID
